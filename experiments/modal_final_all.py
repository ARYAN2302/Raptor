import modal
app = modal.App("raptor-final-all")
...
def run():
    ...
    print("=== FINAL ALL EXPERIMENTS (honest, small-slice) ===")
    # Data: 4 drones
    samples=[]
    for root in ["/data/rfuav/DJI FPV COMBO/DJI FPV COMBO/VTSBW=10", "/data/rfuav/DJI MINI4 PRO/DJI MINI4 PRO/VTSBW=10", "/data/rfuav/FLYSKY FS I6X", "/data/rfuav/FRSKY X9DP2019"]:
        ...
        samples.extend(s)
        for x in s: print(f"RFUAV {x.capture_id} site {x.site_id} cf {x.center_frequency/1e9:.2f}GHz")
    print(f"total RFUAV samples {len(samples)} sites {set(s.site_id for s in samples)}")
    # Full RAPTOR with real Mamba SSM vs GRU baseline
    for temporal in ["gru","mamba"]:
        print(f"\n--- temporal={temporal} ---")
        cfg={"model":{"antennas":1,"d_model":32,"n_latent":8,"n_heads":2,"patch":8,"stride":8,"perceiver_layers":1,"n_queries":4,"temporal": temporal}}
        model=RAPTOR(cfg)
        if torch.cuda.is_available(): model=model.cuda()
        print(f"params {sum(p.numel() for p in model.parameters())}")
        # Stage B: 1 epoch masked recon on 4 drones leakage-safe (train 2 sites, val 2 sites)
        ...
        # Range continuous trajectory (same emitter, 4 windows) with this temporal
        ...
        # train range 1 epoch continuous
    ...
    torch.save({"cfg": cfg}, "/ckpt/final_all.pt")
    ckpt_vol.commit()
    print("final all done — honest: GRU baseline vs Mamba SSM both use continuous trajectory, not independent seeds")
