"""Synthetic coherent IQ generator — bridges missing GT (§6-7). No Sionna required for unit tests."""
import numpy as np
import torch
from ..dataio.canonical import CanonicalSample

def synth_iq(T=4096, E=4, sample_rate=100e6, center_freq=2.4e9, n_emitters=1, snr_db=10, seed=0):
    ...
    for k in range(n_emitters):
        ...
        kvec=np.array([np.cos(el_r)*np.sin(az_r), np.cos(el_r)*np.cos(az_r), np.sin(el_r)])
        delay=pos.dot(kvec)/c
        phase=2*np.pi*center_freq*delay
        vel=rng.uniform(-15,15, size=3)
        ...
        for e in range(E):
            ...
            iq[:,e,0]+= sig.real
            iq[:,e,1]+= sig.imag
        emitters.append(dict(id=k, range=r, azimuth=az, elevation=el, velocity_xyz=vel.tolist(), radial_velocity=float(np.dot(vel,kvec))))
    sig_p=np.mean(iq**2)
    noise_p=sig_p/(10**(snr_db/10)) if n_emitters>0 else 1e-3
...
class SyntheticIQDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, i):
        ...
        return {"iq": torch.from_numpy(iq), "emitter_count": s.emitter_count, "emitters": s.emitters, "antenna_positions": torch.from_numpy(s.antenna_positions)}
